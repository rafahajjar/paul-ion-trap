function comet3n(src, rm, zm, vx, vy, vz, qn, varargin)
...
for ii = 1:numObj
    ...
    gh(ii).ph = plot(0,'LineWidth', tailwidth);
end
...
for tt = 1:max(src(:,4))
    ...
    for ii = 1:length(ObjList)
        ...
        if ind_t <= tailLength
            ...
            set(gh(ObjList(ii)).ph, 'XData', PosList_ID{ObjList(ii)}(ind_t-tailLength:ind_t,1),...
                'YData', PosList_ID{ObjList(ii)}(ind_t-tailLength:ind_t,2),...
                'Zdata', PosList_ID{ObjList(ii)}(ind_t-tailLength:ind_t,3),...
                'color', PosList_ID{ObjList(ii)}(ind_t,5:7));
        end
    end
    drawnow
